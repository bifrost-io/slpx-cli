...
export function resolveToken(name: string): TokenInfo {
  ...
  if (!found) {
    ...
    throw new Error(`Unknown token. Valid: ${valid}`);
  }
  return found;
}

export type RewardIncentiveAsset = "vDOT" | "BNC";

/** vETH: `rewardApy` incentive in vDOT terms; any other vToken: BNC. */
export function rewardApyIncentiveAsset(
  vTokenId: string,
): RewardIncentiveAsset {
  return vTokenId === "vETH" ? "vDOT" : "BNC";
}
